        ...
        <div className="container mx-auto flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-8">
            ...
            <div className="flex items-center gap-4 pl-4 border-l border-[var(--border-color)]">
              ...
              <button onClick={handleLoginClick} className="px-6 py-2 text-white text-xs font-bold rounded-full transition-all hover:scale-105 active:scale-95 shadow-lg" style={{ backgroundColor: "var(--current-accent)" }}>LOGIN</button>
            </div>
          </div>

          {/* Mobile actions: show login, theme toggle and menu on small screens */}
          <div className="flex lg:hidden items-center gap-3">
            ...
            <button onClick={handleLoginClick} className="px-3 py-2 text-white text-xs font-bold rounded-full transition-all hover:scale-105 active:scale-95 shadow-lg" style={{ backgroundColor: "var(--current-accent)" }}>LOGIN</button>

            <button onClick={() => setMobileOpen((v) => !v)} aria-label="Toggle menu" className="p-2 rounded-md hover:bg-[var(--border-color)]">
              {mobileOpen ? (
          ...
          {mobileOpen && (
            <>
              ...
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="fixed top-16 left-0 right-0 z-50 bg-[var(--bg-primary)] border-t border-[var(--border-color)] lg:hidden">
                <div className="flex flex-col divide-y divide-[rgba(255,255,255,0.03)]">
                  {navItems.map((item) => (
                    <button key={item.id} onClick={() => scrollTo(item.id)} className={`text-left w-full px-6 py-4 text-[var(--text-primary)] hover:bg-[rgba(255,255,255,0.02)]`}>{item.label}</button>
                  ))}
                  <div className="flex items-center gap-4 px-6 py-4">
                    <button onClick={() => setAccent('orange')} className="w-4 h-4 rounded-full" style={{ backgroundColor: 'var(--accent-orange)' }} />
                    <button onClick={() => setAccent('teal')} className="w-4 h-4 rounded-full" style={{ backgroundColor: 'var(--accent-teal)' }} />
                    <button onClick={() => setAccent('green')} className="w-4 h-4 rounded-full" style={{ backgroundColor: 'var(--accent-green)' }} />
                    <div className="flex-1" />
                    <button onClick={handleLoginClick} className="px-4 py-2 text-white text-xs font-bold rounded-full" style={{ backgroundColor: 'var(--current-accent)' }}>LOGIN</button>
                  </div>
                </div>
              </motion.div>
            </>
