...
const Calendar: React.FC = () => {
  ...
  const refreshData = () => {
    ...
    // Holidays (overrides working Saturday if same date)
  ...
  const generateCalendarDays = () => {
    ...
    for (let d = 1; d <= lastDate; d++) {
      ...
      // Regular weekends (non-working Saturdays)
  ...
  const handleDayClick = (day: any) => {
    if (day.record) {
      ...
      // If there are events but no attendance record
      setSelectedRecord({
            ...
            >
              <div className="flex justify-between items-start mb-1">
                <div className="flex flex-col">
                  ...
                  {/* Event count indicator - Mobile shows just dot */}
                  {day.events.length > 0 && (
                ...
                <div className="space-y-2 sm:space-y-3">
                  <div className="grid grid-cols-2 gap-2 sm:gap-3 lg:gap-4">
                    <div className="bg-slate-50 p-2 sm:p-3 lg:p-4 rounded-lg sm:rounded-xl lg:rounded-2xl border border-slate-100 text-center">
                      ...
                      <p className={`text-xs sm:text-sm font-black ${selectedRecord.isLate ? 'text-amber-600' : 'text-slate-800'}`}>
