...
import {
  Calendar, CheckCircle, Clock, ArrowRight, AlertCircle, XCircle, Plus, Filter, Download, User, FileText, MoreVertical, Eye, Edit, Trash2, X, Loader2, Send, Calculator
} from 'lucide-react';
...
// Copy of Employee Leave.tsx, adapted for admin
const AdminLeave: React.FC = () => {
  ...
  const fetchMyLeaves = async (statusParam?: string) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8085/leave-requests/my-leaves', {
        method: 'GET',
        credentials: 'include',
      });
      ...
      const msg = err?.message || 'Failed to fetch leave requests';
      toast.error(msg);
    } finally {
